...
def prof_ver_treino(id_aluno_user, categoria):
    try:
        ...
        if not treino:
            return jsonify({"exercicios": []})
        exercicios = [
            {"nome": e.nome_exercicio, "series": e.series, "reps": e.repeticoes, "carga": e.carga}
...
def meu_treino(categoria):
    ...
    treino = Treino.query.filter_by(id_aluno=aluno.id_aluno, nome_treino=categoria.upper()).first()
    if not treino:
        return jsonify({"mensagem": "// NENHUM PROTOCOLO ENCONTRADO."}), 404
 
    exercicios = [
        {"id_item": e.id_item_treino, "nome": e.nome_exercicio, "series": e.series, "reps": e.repeticoes, "carga": e.carga}
        for e in ItemTreino.query.filter_by(id_treino=treino.id_treino).all()
    ]
...
def remover_conexao(id_vinculo):
    ...
    vinculo = db.session.execute(
        text("SELECT id_aluno FROM vinculo_professor_aluno WHERE id_vinculo = :id"),
        {"id": id_vinculo}
    ).fetchone()
    if not vinculo:
    ...
    if perfil == 'admin' or user_id == vinculo[0]:
        db.session.execute(
            text("DELETE FROM vinculo_professor_aluno WHERE id_vinculo = :id"),
            {"id": id_vinculo}
        )
        db.session.commit()
        return jsonify({"sucesso": True})
...
def atualizar_perfil_completo():
    ...
    try:
        if 'foto' in request.files:
            ...
            if file and ext in {'png', 'jpg', 'jpeg', 'gif'}:
...
def criar_produto():
    ...
    try:
        novo = Produto(
            ...
            preco=data.get('price'), status=data.get('status'),
            icone=data.get('icon'), quantidade=data.get('quantity')
        )
        db.session.add(novo)
        db.session.commit()
