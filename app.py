...
def login():
    if 'usuario_id' in session: return redirect(url_for('index'))
...
def cadastro():
    data = request.get_json()
    try:
        if Usuario.query.filter_by(email=data.get('email')).first(): return jsonify({'sucesso': False, 'mensagem': 'EMAIL_JÁ_REGISTRADO'}), 400
        if Usuario.query.filter_by(cpf=data.get('cpf')).first(): return jsonify({'sucesso': False, 'mensagem': 'CPF_JÁ_REGISTRADO'}), 400
        data_nasc_mysql = datetime.strptime(data.get('data_nascimento'), '%d/%m/%Y').date()
        novo_usuario = Usuario(
            nome=data.get('nome'), email=data.get('email'), senha=generate_password_hash(data.get('senha')),
            cpf=data.get('cpf'), perfil='aluno', data_nascimento=data_nasc_mysql,
...
def atualizar_perfil_completo():
    ...
    try:
        ...
        if request.form.get('data_nascimento'): usuario.data_nascimento = datetime.strptime(request.form.get('data_nascimento'), '%Y-%m-%d').date()
        ...
        aluno = Aluno.query.filter_by(id_usuario=usuario.id_usuario).first()
        if aluno and request.form.get('objetivo'): aluno.objetivo = request.form.get('objetivo')
