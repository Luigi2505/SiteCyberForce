...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            print(f'ERRO DB: {e}')
            return jsonify({'sucesso': False, 'mensagem': 'ERRO_DE_CONEXÃO_COM_BANCO'}), 503
...
def cadastro():
    ...
    try:
        ...
        data_nasc_mysql = datetime.strptime(data.get('data_nascimento'), '%d/%m/%Y').date()
        novo_usuario = Usuario(
            nome=data.get('nome'), email=data.get('email'),
            senha=generate_password_hash(data.get('senha')),
            cpf=data.get('cpf'), perfil='aluno',
            data_nascimento=data_nasc_mysql,
            genero=data.get('genero'), peso=data.get('peso'), altura=data.get('altura')
        )
        db.session.add(novo_usuario)
        db.session.flush()
        db.session.add(Aluno(id_usuario=novo_usuario.id_usuario))
        db.session.commit()
...
def vincular_aluno():
    ...
    dados = request.json
    try:
        db.session.execute(text("INSERT INTO vinculo_professor_aluno (id_professor, id_aluno) VALUES (:prof, :aluno)"),
                           {"prof": session.get('usuario_id'), "aluno": dados.get('id_aluno')})
        db.session.commit()
...
def prof_desvincular_aluno(id_aluno):
    if session.get('perfil') not in ['treinador', 'admin']: return jsonify({'erro': 'Acesso negado'}), 403
    db.session.execute(text("DELETE FROM vinculo_professor_aluno WHERE id_professor = :prof AND id_aluno = :aluno"),
                       {"prof": session.get('usuario_id'), "aluno": id_aluno})
    db.session.commit()
    return jsonify({"sucesso": True})
...
def prof_ver_treino(id_aluno_user, categoria):
    try:
        ...
        exercicios = [{"nome": e.nome_exercicio, "series": e.series, "reps": e.repeticoes, "carga": e.carga}
                      for e in ItemTreino.query.filter_by(id_treino=treino.id_treino).all()]
...
def salvar_treino():
    try:
        ...
        primeiro_prof = db.session.execute(
            text("SELECT id_professor FROM vinculo_professor_aluno WHERE id_aluno = :aluno ORDER BY data_vinculo ASC LIMIT 1"),
            {"aluno": id_aluno_user}).fetchone()
        ...
        for ex in dados.get('exercicios', []):
            ...
            db.session.add(ItemTreino(
                id_treino=novo_treino.id_treino, nome_exercicio=nome_ex,
                series=int(ex.get('series') or 0), repeticoes=int(ex.get('reps') or 0),
...
def meu_treino(categoria):
    ...
    exercicios = [{"id_item": e.id_item_treino, "nome": e.nome_exercicio, "series": e.series, "reps": e.repeticoes, "carga": e.carga}
                  for e in ItemTreino.query.filter_by(id_treino=treino.id_treino).all()]
...
def listar_produtos():
    ...
    return jsonify([{"id": p.id_produto, "name": p.nome, "brand": p.marca, "cat": p.categoria,
                     "price": p.preco, "status": p.status, "icon": p.icone, "quantity": p.quantidade} for p in produtos])

@app.route('/api/produtos', methods=['POST'])
def criar_produto():
    if session.get('perfil') not in ['admin', 'treinador']: return jsonify({"sucesso": False, "mensagem": "Acesso Negado"}), 403
    data = request.get_json()
    try:
        db.session.add(Produto(nome=data.get('name'), marca=data.get('brand'), categoria=data.get('cat'),
...
def alterar_quantidade(id_produto):
    if session.get('perfil') not in ['admin', 'treinador']: return jsonify({"sucesso": False}), 403
    produto = Produto.query.get(id_produto)
    if produto:
        produto.quantidade = request.get_json().get('quantity')
