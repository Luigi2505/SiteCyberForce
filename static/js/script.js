    ...
    fetch("/api/usuario/perfil")
      ...
      .then((data) => {
        if (data && data.foto_url) {
          document.getElementById("header-profile-img").src = data.foto_url;
        }
      })
      .catch(() => {});
  }
...
function mostrarAviso(msg, isError = false) {
    ...
    toast.style.boxShadow = isError ? '0 0 20px rgba(255,34,68,0.3)' : '0 0 20px rgba(0,212,255,0.3)';
...
function showSection(sectionId) {
    ['home','home-section','perfil','logbook','painel-professor','store','schedule','panel-login'].forEach(id => {
    ...
    if(sectionId === 'painel-professor') { carregarAlunosDoProfessor(); carregarAdminConexoes(); }
...
async function carregarDadosPerfil() {
    try {
        ...
        if(response.ok) {
            const mapa = { nome:'nome', cpf:'cpf', email:'email', objetivo:'objetivo', peso:'peso', altura:'altura', genero:'genero', data:'data_nascimento' };
    ...
    } catch(e) { mostrarAviso('// ERRO NA CONEXÃO NEURAL.', true); }
}
...
async function salvarPerfil() {
    ...
    ['nome','cpf','email','genero','peso','altura','objetivo'].forEach(campo => {
    ...
    if(result.sucesso) { mostrarAviso('// PROTOCOLO ATUALIZADO COM SUCESSO.'); setTimeout(() => location.reload(), 2000); }
...
function previewFoto(input) {
    if(input.files && input.files[0]) {
        ...
        reader.onload = e => { const img = document.getElementById('perfil-img-display'); if(img) img.src = e.target.result; };
        reader.readAsDataURL(input.files[0]);
    }
}

function deletarConta() {
    cyberConfirm('⚠️ TEM CERTEZA? SEU REGISTRO SERÁ APAGADO PERMANENTEMENTE DA CYBERFORCE.', () => {
...
function renderizarListaAlunos(lista) {
    ...
    grid.innerHTML = '';
    if(lista.length === 0) {
        grid.innerHTML = '<p class="field-label" style="color:var(--text-dim);grid-column:1/-1">// NENHUM ALUNO VINCULADO. USE + RECRUTAR ALUNO.</p>';
        return;
    }
    lista.forEach(a => {
...
async function buscarNovoAluno() {
    ...
    lista.innerHTML = '';
    if(alunos.length === 0) { lista.innerHTML = '<p style="color:var(--neon-red);">// NENHUM ALUNO ENCONTRADO.</p>'; return; }
    alunos.forEach(a => {
...
async function buscarTodosAlunos() {
    ...
    try {
        ...
        const alunos = await res.json();
        lista.innerHTML = '';
        if(alunos.length === 0) { lista.innerHTML = '<p style="color:var(--neon-red);">// NENHUM ALUNO CADASTRADO.</p>'; return; }
        alunos.forEach(a => {
    ...
    } catch(e) { lista.innerHTML = '<p style="color:var(--neon-red);">// ERRO DE CONEXÃO.</p>'; }
}
...
async function vincularAluno(id) {
    ...
    const data = await res.json();
    if(res.ok) { mostrarAviso(data.mensagem); document.getElementById('resultado-busca').style.display = 'none'; carregarAlunosDoProfessor(); }
    else mostrarAviso(`// ERRO: ${data.mensagem}`, true);
}

function desvincularAlunoProfessor(id_aluno) {
    cyberConfirm('⚠️ Deseja realmente remover este aluno da sua tutela?', async () => {
        const res = await fetch(`/api/professor/desvincular_aluno/${id_aluno}`, {method:'DELETE'});
        if(res.ok) { mostrarAviso('// ALUNO REMOVIDO.'); carregarAlunosDoProfessor(); }
...
async function carregarTreinoProfessor() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--cyan)">// EXTRAINDO DADOS...</td></tr>';
    try {
    ...
    } catch(e) { tbody.innerHTML = ''; adicionarLinhaExercicio(); }
}
...
async function salvarTreinoCompleto() {
    ...
    try {
        ...
        const data = await res.json();
        if(res.ok && data.sucesso) { mostrarAviso(data.mensagem); fecharCriadorTreino(); carregarAlunosDoProfessor(); }
        else mostrarAviso(`// ERRO: ${data.mensagem}`, true);
    } catch(e) { mostrarAviso('// ERRO CRÍTICO. Tente novamente.', true); fecharCriadorTreino(); }
}
